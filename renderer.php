...
class mod_attforblock_renderer extends plugin_renderer_base {
    ...
    protected function render_sess_manage_table(attforblock_manage_data $sessdata) {
        ...
        $url = new moodle_url('/mod/attforblock/manage.php', array('id' => $sessdata->cm->id));
        
        $table = new html_table();
        
        switch ($sessdata->params->collapsetype) {
          case 1:
              $table->size = array('1px', '', '1px', '1px', '*', '1px', '1px');
              $url->param('collapsetype', 2);
              $icon = $this->output->action_icon($url, new pix_icon('t/switch_minus', 'Hide'));
              $t = get_string('sessiontypeshort', 'attforblock').$icon;
            break;
            
          case 2:
              $table->size = array('1px', '1px', '1px', '1px', '*', '1px', '1px');
              $url->param('collapsetype', 1);
              $t = $this->output->action_icon($url, new pix_icon('t/switch_plus', 'Show'));
            break;
        }
